...
def render_patient_card(p, ordem=None):
    ...
    st.markdown(f"""
    <div class="patient-card {'padrao' if p['score']<55 else 'alto' if p['score']<80 else 'critico'} {'done' if inactive else ''}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
            <div style="display:flex;gap:10px;align-items:flex-start">
                <div style="width:28px;height:28px;border-radius:50%;background:{score_bg(p['score'])};
                     border:2px solid {sc};display:flex;align-items:center;justify-content:center;
                     color:{sc};font-weight:800;font-size:12px;flex-shrink:0">{ord_label}</div>
                <div>
                    <div style="font-family:monospace;font-size:11px;color:#6b6560;margin-bottom:3px">{p['code']}</div>
                    <div style="display:flex;gap:4px;flex-wrap:wrap">
                        {condition_badges(p)} {status_html}
                    </div>
                </div>
            </div>
            <div style="text-align:right;flex-shrink:0">
                ...
                <div style="font-size:10px;color:{'#c0392b' if p['dias_sem_visita']>30 else '#9c948c'};margin-top:2px">
                    {p['dias_sem_visita']}d sem visita
                </div>
            </div>
        </div>
        <div style="margin-top:7px;padding-top:6px;border-top:1px solid #e2ddd6;
             display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:11px;color:#6b6560">📍 {p['endereco']}</span>
            <div style="display:flex;gap:8px;align-items:center">
                <span style="font-size:10px;color:#1d4ed8">F:{len(fichas)}</span>
                {"<span style='font-size:10px;color:#1d4ed8;font-weight:700'>A:" + str(n_audios) + "</span>" if n_audios > 0 else ""}
...
def render_audio_recorder(p):
    ...
    # Simulated recording (browser mic requires JS — placeholder for Streamlit)
    st.markdown("---")
