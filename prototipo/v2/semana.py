...
from utils.helpers import is_padrao, get_status, set_status, score_label, score_color
...
def render_semana():
    ...
    # Bulk WPP for whole week
    upcoming_padrao = [
        p for p in PATIENTS
        if p["week_day"] > 0
        and is_padrao(p)
        and not get_status(p["id"])
        and not st.session_state.wpp_sent_week.__contains__(p["id"])
    ]
    if upcoming_padrao:
        st.markdown(f"**💬 WPP em massa — semana toda**")
