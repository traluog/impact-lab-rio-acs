...
def render_header():
    ...
    # Pre-build to avoid blank lines inside the HTML block (CommonMark ends HTML
    # blocks on blank/whitespace-only lines, causing raw text rendering).
    urgencia_badge = (
        f"<span style='background:#c0392b;color:#fff;border-radius:8px;"
        f"padding:6px 10px;font-size:13px;font-weight:700'>🔔 {urgencias}</span>"
        if urgencias > 0 else ""
    )
...
<div class="acs-header">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
    <div>
      <div style="font-size:10px;color:#94a3b8;letter-spacing:1.4px;text-transform:uppercase;margin-bottom:2px">ACS · {ACS_TEAM}</div>
